...
- **Human operator:** approves each numbered phase and any additional destructive action required by the safety boundary.

## Lifecycle

1. Detect the next incomplete phase with `make phase-status`.
2. Create or review `.ai/phases/phase-XX.yaml`.
...
5. Codex implements only the approved contract.
6. Run `make phase-validate PHASE=<n>` when a phase-specific validator exists.
7. Review the diff/PR against the contract. Classify findings as `BLOCKER`, `WARNING`, or `IMPROVEMENT`.
8. If fixes are needed, save actionable findings to a small text/Markdown file and generate `MODE=fix`; do not resend broad repository context.
9. Re-run deterministic validation.
10. Human approves merge. Update tracker evidence only after successful validation and merge.
